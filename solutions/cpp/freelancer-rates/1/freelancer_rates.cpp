...
double apply_discount(double before_discount, double discount) {
    ...
    return before_discount * (100.0 - discount) / 100.;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double rate = apply_discount(daily_rate(hourly_rate)*22.0, discount);
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double one_day = apply_discount(daily_rate(hourly_rate), discount);
    // std::cout << one_day;
    return budget / static_cast<int>(one_day);
}
